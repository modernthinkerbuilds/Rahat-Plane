...
# ─── Full-route delegation endpoints (parity with native_client) ────────────
# These expose Kobe/Fraser/Huberman's complete route() over HTTP so the
# runner's delegation paths work in HTTP mode. Kept in sync with
# new_plane.miya_runner.{native_client,adapter_client}; a parity contract
# test guards the surface.
class _RouteReq(_Envelope):
    message: str
    chat_id: str | None = None


@app.post("/kobe/route", dependencies=[Depends(require_token)])
def kobe_route(req: _RouteReq) -> dict:
    try:
        from agents.the_scientist import handler as kobe_handler
        text = kobe_handler.route(req.message)
        return {"trace_id": _trace(req),
                "result": {"text": text or "", "path": "kobe_route"}}
    except Exception as e:
        return {"trace_id": _trace(req), "error": f"{type(e).__name__}: {e}"}


@app.post("/fraser/route", dependencies=[Depends(require_token)])
def fraser_route(req: _RouteReq) -> dict:
    try:
        from agents.fraser import handler as fraser_handler
        text = fraser_handler.route(req.message, chat_id=req.chat_id)
        if not isinstance(text, str):
            text = str(text)
        return {"trace_id": _trace(req),
                "result": {"text": text, "path": "fraser_route"}}
    except Exception as e:
        return {"trace_id": _trace(req), "error": f"{type(e).__name__}: {e}"}


@app.post("/huberman/route", dependencies=[Depends(require_token)])
def huberman_route(req: _RouteReq) -> dict:
    # Parked agent: delegates to Kobe's mesh with an explicit @huberman tag
    # (mirrors native_client.huberman_route).
    try:
        from agents.the_scientist.handler import route as kobe_route_fn
        text = kobe_route_fn(f"@huberman {req.message}")
        return {"trace_id": _trace(req),
                "result": {"text": text or "", "path": "huberman_route"}}
    except Exception as e:
        return {"trace_id": _trace(req), "error": f"{type(e).__name__}: {e}"}
