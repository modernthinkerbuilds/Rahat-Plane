...
Runtime-agnostic by design: Rahat runs on its own local Python runtime today; an [OpenClaw](https://github.com/openclaw/openclaw) adapter is kept ready as **one integration target, not a foundation** (see ADR-014) — credit and respect to that team. Architecture and documentation: MIT.
