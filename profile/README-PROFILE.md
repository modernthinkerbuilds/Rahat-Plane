...
- **The Scientist agent in production** through the new contract — full memory adapter (`goal`, `plan`, `commitment`, `tier_change`), full 25-tool reasoner, daily real use through Telegram.
- **Bajrangi (recovery agent) shipped as a stub** to prove the substrate composes for non-Scientist agents — memory adapter ready (`recovery_protocol`, `sleep_concern`), Charter enforcement wired.
...
Plus one piece of infrastructure that isn't an agent — **The Charter** — the policy plane every outbound write-tool passes through. Quiet hours, HRV-red blocks, family-priority overrides: written once, applied uniformly. Bajrangi advises; the Charter enforces. The separation between advisor and enforcer is what keeps governance consistent as the mesh grows past three agents.

### The roadmap is a hypothesis test

Six agents in the Now window (~6 months, scaling toward ~20 by month 12), each shipped against the same agent contract: entity types + adapter + tool registry, ~1 day per agent. **Curriculum** (toddler + newborn developmental phases), **Coach (Fraser)** (CrossFit programming + load auditing), **Appointments** (calendar + scheduling), then concierge-class specialists — **The Voyager** (travel), **Annapurna** (pantry & grocery), **The Barista** (espresso ritual + bean inventory), **Ustad** (asynchronous guitar audit).
...
Bay Area PM. Hyderabad roots. Husband, father of two (toddler + newborn). The "Bajrangi" agent exists because there are days when pulling heavy is the wrong call and I needed an agent honest enough to say so. The whole point of Rahat is to build the kind of agents I actually want pointed at my own life — health, household, training, and (eventually) the espresso bar.
