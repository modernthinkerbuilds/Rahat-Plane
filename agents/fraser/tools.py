...
HBP_CUE         = "HBP Rule — exhale on the up portion. Never max Valsalva."
...
# that apply — pressing movements get neck-guard + HBP; squat patterns
# get ankle-check + HBP; pulls get hunch + neck-guard. Free-text per
# the Gemini transcript: "every heavy lift carries an explicit exhale
# cue. Never max Valsalva."
CUE_TABLE: dict[str, tuple[str, ...]] = {
    # Pressing
    "bench":          (NECK_GUARD_CUE, HBP_CUE),
    "strict_press":   (NECK_GUARD_CUE, HBP_CUE),
    "push_press":     (NECK_GUARD_CUE, HBP_CUE),
    "floor_press":    (NECK_GUARD_CUE, HBP_CUE),
    "dumbbell_press": (NECK_GUARD_CUE, HBP_CUE),
    # Pulling
    "deadlift":       (HUNCH_CUE, HBP_CUE),
    "sumo_deadlift":  (HUNCH_CUE, HBP_CUE),
    "pull_up":        (HUNCH_CUE,),
    "chin_up":        (HUNCH_CUE,),
    "trx_row":        (HUNCH_CUE,),
    "ring_row":       (HUNCH_CUE,),
    # Squat patterns
    "back_squat":     (ANKLE_CHECK_CUE, HBP_CUE),
    "front_squat":    (ANKLE_CHECK_CUE, HBP_CUE),
    "thruster":       (ANKLE_CHECK_CUE, NECK_GUARD_CUE, HBP_CUE),
    # Olympic
    "clean":          (HUNCH_CUE, ANKLE_CHECK_CUE, HBP_CUE),
    "snatch":         (HUNCH_CUE, ANKLE_CHECK_CUE, HBP_CUE),
    # Warm-up
    "face_pull":      (HUNCH_CUE,),
    "cat_cow":        (HUNCH_CUE,),
...
BENCHMARKS: dict[str, _BenchmarkSpec] = {
    ...
    "diane": _BenchmarkSpec(
        ...
        movements=[
            ("deadlift", "21-15-9", 102.0, "deadlift"),
...
def compute_target_weight(lift: str, percentage: float, one_rm_kg: float,
    ...
    Examples:
        >>> compute_target_weight("deadlift", 70, 155.0)
        107.5
...
def lookup_movement_cues(movement: str) -> list[str]:
    ...
    guard cues, heavy lifts carry the HBP rule.

    Unknown movements get the HBP rule by default — it's the most
...
__all__ = [
    ...
    "HUNCH_CUE", "NECK_GUARD_CUE", "HBP_CUE", "ANKLE_CHECK_CUE",
    "CUE_TABLE",
    "BENCHMARKS",
    # Dataclasses
