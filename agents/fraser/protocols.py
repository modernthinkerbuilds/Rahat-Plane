...
class StrengthLift:
    ...
    schedule and the cardio-caution cue inline (spec §5 item 5 + §5 item 3)."""
    name: str
    working_sets: int = 1
    working_reps: int = 1
...
TOOL_CATALOG: tuple[ToolManifest, ...] = (
    ...
    ToolManifest(
        ...
        description=(
            ...
            "category (pressing → neck-guard + cardio-caution; squat → ankle-"
            "check + cardio-caution; pull → hunch; Olympic → all three + cardio-caution). "
            "Surface the returned cues in the WORKOUT CARD's NOTES "
            "section. Unknown movements get the cardio-caution rule by default "
        ...
        returns_schema={
            "type": "array",
            "description": (
                "List of cue strings. Always non-empty (at least the cardio-caution "
                "fallback)."
            ),
        },
    ),
