...
# ── Athlete constants (Venkat-specific, tuned from coaching history) ─────
...
TARGET_LBS        = float(os.getenv("TARGET_LBS", "185"))
...
def replan_week(monday: datetime, *, force: bool = False) -> list[dict]:
    ...
    Pattern is fixed at 3 PRVN CrossFit + 1 Zone-2 10K + 3 active-rest.
    CrossFit days come from the gym schedule, filtered against the user's
    movement blacklist (no partner / handstand / muscle-up / OHS / snatch
    in the strength block). Zone-2 prefers Saturday, falling back to the
    latest unused weekday so it still leaves room for recovery.
    """
    ...
    try:
        ...
        # Pick CF days from the gym schedule.
        gym_days = parse_gym_plan()
        elig = eligible_cf_days(gym_days)
        cf_picks: list[tuple[int, str]] = []
        for d in elig:
            wd = WEEKDAY_INDEX.get(d.weekday[:3])
            if wd is not None and len(cf_picks) < 3:
                cf_picks.append((wd, d.label))
        cf_weekdays = {wd for wd, _ in cf_picks}

        # Z2 day: Sat preferred, otherwise the latest unused weekday.
        z2_wd = None
        for candidate in [Z2_PREFERRED_WEEKDAY, 6, 4, 3, 2, 1, 0]:
            if candidate not in cf_weekdays:
                z2_wd = candidate
                break
        ...
        for wd in range(7):
            if wd in cf_weekdays:
                day_type = "cf"
                gym_label = next(label for w, label in cf_picks if w == wd)
            elif wd == z2_wd:
                day_type = "z2"
                gym_label = None
            else:
            ...
            plan.append({
                ...
                "day_type": day_type,
                "gym_label": gym_label,
                "target_kcal": day_type_target(day_type),
            })
...
def handle_show_plan() -> str:
    ...
    lines = [f"*Weekly plan* — tier `{tier}`, target {fmt_kcal(target)}"]
    for row in plan:
        marker = "▶" if row["weekday"] == today_idx else " "
        name = WEEKDAY_NAME[row["weekday"]]
        kind = DAY_TYPE_LABEL[row["day_type"]]
        gym = f" ({row['gym_label']})" if row["gym_label"] else ""
        actual = burn_for_date(week_bounds()[0] + timedelta(days=row["weekday"]))
        actual_s = f" — burned {fmt_kcal(actual)}" if actual > 0 else ""
        lines.append(
            f"{marker} {name}: {kind}{gym} → ideal {fmt_kcal(row['target_kcal'])}{actual_s}")
...
TODAY_TARGET_RE = re.compile(r"\b(today.?s?\s+target|ideal today|today.?s?\s+ideal|day target)\b", re.I)
...
DECIDE_RE   = re.compile(r"\b(should I (run|do crossfit|do a wod)|"
                         r"run (or|vs) (crossfit|wod|workout)|"
                         r"crossfit (or|vs) run)\b", re.I)
...
def maybe_walk_nudge() -> str | None:
    ...
    return (
        ...
        f"Today: {fmt_kcal(today_burn)} vs pace {fmt_kcal(pace)}. "
        "Take a 10–15 min walk this hour."
    )
