...
def _db():
    ...
    Per PRD §4.1 the ledger is a State-Machine with four logical stores:
      ...
      • intents       — hard-coded North Stars (80 kg, 200 kg deadlift)
...
def _spaced_pick(candidates: list[int], n: int) -> list[int]:
    """Pick `n` weekdays (0=Mon..6=Sun) from `candidates`, spaced for recovery.

    Owner 2026-06-18: when more blacklist-clean gym days are available than the
    cadence needs, the tie-breaker is RECOVERY SPACING — maximize the smallest
    gap between consecutive sessions so three CF days aren't stacked back-to-
    back (the 3rd would be junk). Tie-break toward the wider total spread, then
    the earlier set (deterministic). Returns days sorted ascending.

    Honest-proxy note: with no per-WOD burn estimate yet, "maximize calorie
    efficiency" = clean (uncompromised) days, spaced for recovery. When the
    Fraser per-WOD effort estimator lands, this ranks by expected burn instead.
    """
    import itertools
    ...
    for combo in itertools.combinations(cands, n):
        ...
        key = (min(gaps), sum(gaps), tuple(-c for c in combo))
        if best_key is None or key > best_key:
            best_key, best = key, list(combo)
    return best if best is not None else cands[:n]
...
def replan_week(monday: datetime, *, force: bool = False) -> list[dict]:
    ...
    try:
        ...
        if forced_orig:
            ...
            # Normal path: gym plan synced + has 3+ CF-eligible days. When
            # MORE than 3 are clean, recovery-spacing is the tie-breaker
            # (owner 2026-06-18) — pick the 3 most spread-out clean days
            # rather than the earliest 3.
            cf_wds = _spaced_pick(eligible_wds, 3)
        else:
        ...
        # Rescale per-day ONLY to honor an EXPLICIT weekly commitment
        # ("I'll do 5,500 this week"). 2026-06-18 (owner, B model): the GOAL
        # no longer rescales the per-day ideals — that produced the
        # 1,225/1,350/550 nonsense by squashing the tier template onto a
        # goal-derived weekly number. The goal still drives PACE FEEDBACK via
        # weekly_target() (behind/ahead + make-up suggestions); a CF day just
        # keeps its fixed template burn. Tier-default + goal-only weeks keep
        # the documented tier template untouched.
        committed = _active_weekly_commitment()
        effective_weekly = committed
