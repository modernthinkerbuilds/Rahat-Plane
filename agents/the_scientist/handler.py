...
def handle_pre_fuel(minutes_to_workout: int = 75) -> str:
    ...
    return (
        ...
        "2. Half a small GF roti + thin honey\n"
...
def handle_post_recovery() -> str:
    return (
        ...
        "• Foam roller thoracic 2 min — opens up posture\n"
...
def handle_show_plan(next_week: bool = False) -> str:
    ...
    # in the migration — tell me which days to train and surface the
    # blocked days + the one-word fix — instead of silently
    # under-scheduling or printing a blocked WOD on a rest row with no
    # context. Additive (sits above the day grid); the per-day sub-line
    # rendering below is unchanged. Only when the gym is actually synced;
    # the "no plan synced" warning already covers the empty case.
    if has_synced_data:
        cf_wds = sorted(r["weekday"] for r in plan if r["day_type"] == "cf")
        blocked_bits: list[tuple[int, str]] = []
        for bwd in sorted(wd_to_gym):
            gd = wd_to_gym[bwd]
            if bwd in cf_wds or not gd.blockers:
                continue
            mv = gd.blockers[0].split(" (")[0]
            blocked_bits.append((bwd, mv))
        if cf_wds:
            go = ", ".join(WEEKDAY_NAME[wd] for wd in cf_wds)
            lines.append(
                f"_🗓 Train these days: *{go}* — CrossFit, spaced for "
                f"recovery._")
            if blocked_bits:
                named = ", ".join(
                    f"{WEEKDAY_NAME[wd]} ({mv})" for wd, mv in blocked_bits)
                first_mv = blocked_bits[0][1]
                lines.append(
                    f"_↳ Blocked (not assigned): {named} — `tolerate "
                    f"{first_mv}` to open it as a gym day._")
            lines.append("")
    ...
    for row in plan:
        ...
        if gym_day.blockers:
            ...
            hint = (f"(gym runs this — blocked by {blocker_first}, not "
                    f"assigned; `tolerate {blocker_first}` to use it as "
                    f"a gym day)")
        else:
...
# ── Deterministic WOD scaling (Bug 2026-06-14) ────────────────────────
# Gym WODs were shown un-scaled, so the athlete had to translate his
# no-rig / no-med-ball / no-rope setup + mobility cues himself every time.
# These rules map movement keywords in the gym WOD text to the fixed subs
# already encoded in core.athlete_profile. No LLM — pure profile data.
_WOD_SCALE_RULES = [
    (re.compile(r"\bwall\s*balls?\b", re.I),
     "Wall Balls", "DB / Goblet Thrusters (no med ball)"),
    (re.compile(r"\b(?:bar\s*)?muscle[-\s]?ups?\b", re.I),
     "Muscle-Ups", "Heavy DB Rows / Ring Rows (no rig)"),
    (re.compile(r"\b(?:chest[-\s]?to[-\s]?bar|c2b|pull[-\s]?ups?)\b", re.I),
     "Pull-Ups / C2B", "Heavy DB Rows / Bent-Over Barbell Rows (no rig)"),
    (re.compile(r"\b(?:toes[-\s]?to[-\s]?bar|t2b)\b", re.I),
     "Toes-to-Bar", "Hollow Rocks / V-Ups (no rig)"),
    (re.compile(r"\b(?:handstand|hspu|wall\s*walks?)\b", re.I),
     "Handstand / HSPU / Wall Walk", "Pike Push-ups (blacklist + neck-safe)"),
    (re.compile(r"\b(?:double[-\s]?unders?|du's?|dus)\b", re.I),
     "Double Unders", "Bike / Penguin Jumps (no rope)"),
    (re.compile(r"\boverhead\s*squats?\b|\bohs\b", re.I),
     "Overhead Squat", "Front Squat (blacklist)"),
]

_WOD_STANDING_CUES = (
    "Heel lift on all squats · KB swings eye-level only · "
    "exhale on effort (no breath-holding)."
)


def _scale_wod_for_profile(body: str) -> str:
    """Return a deterministic '🔧 Scaled for you' block for a gym WOD,
    derived from the athlete's fixed equipment/blacklist subs. Never
    raises — scaling must never break the WOD render. Toggle off with
    RAHAT_WOD_SCALE=0."""
    if os.getenv("RAHAT_WOD_SCALE", "1").lower() in ("0", "false", "no"):
        return ""
    try:
        if not body:
            return ""
        hits, seen = [], set()
        for rx, label, repl in _WOD_SCALE_RULES:
            if label not in seen and rx.search(body):
                hits.append(f"• {label} → {repl}")
                seen.add(label)
        if not hits:
            return f"\n\n🔧 *Scaled for you*\n_{_WOD_STANDING_CUES}_"
        return ("\n\n🔧 *Scaled for you*\n" + "\n".join(hits) +
                f"\n_{_WOD_STANDING_CUES}_")
    except Exception as e:  # noqa: BLE001 — scaling is best-effort
        print(f"[handle_gym_wod_on] scale block failed: {e}")
        return ""
...
def route(msg: str) -> str:
    ...
    # 3. NL intent layer (ADR-017). Runs ONLY here — after the
    # deterministic dispatcher returned None and delegation passed — so it
    # can never override an exact route; it only catches PARAPHRASES of
    # Kobe's read intents that would otherwise spend a reasoner round-trip.
    # Flag-gated (RAHAT_INTENT_LAYER, default OFF) and read-only by
    # construction; it ABSTAINS (returns None) on any doubt and we fall
    # through to the reasoner unchanged. See core/intent_layer.py.
    try:
        from core import intent_layer as _il
        if _il.enabled():
            _il.ensure_registered()
            _intent_reply = _il.route(msg)
            if _intent_reply is not None:
                return _intent_reply
    except Exception as _ie:
        # Never let the intent layer take down the bot — fall through.
        print(f"[scientist.route] intent layer error, falling back: {_ie}")
