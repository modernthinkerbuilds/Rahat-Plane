...
**Author:** Alex Rivera, with architecture review by Claude (L8 framing)
...
The reference user is a fictional persona, "Alex Rivera" — a busy knowledge worker balancing a serious fitness goal with family and a demanding job. The persona has a CrossFit habit, a body-recomposition target, and months of detailed coaching history that informs Kobe's protocols (a locked weekly loss rate, a fixed intake target, a weekly active-burn target, and a 3 CrossFit + 1 Zone-2 + 3 active-rest cadence). All concrete personal values live only in the gitignored vault profile; the values in this repo are illustrative.
