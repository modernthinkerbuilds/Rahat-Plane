...
# ═══════════════════════════════════════════════════════════════════════
# §2.2 — KOBE_GEMINI_CHAT_REFERENCE.md cases (2026-06-14)
#
# New coverage formalizing the Kobe ground-truth doc: goal-trajectory
# refusal MATH, HRV edge cases beyond band lookup, and multi-turn arcs.
# Deterministic cases are pinned GREEN; genuinely-new behaviours the doc
# specifies but the agent doesn't implement yet are xfail(strict=False)
# with a reference to the KC case so they flip to XPASS when built.
# ═══════════════════════════════════════════════════════════════════════
class TestKobeRefGoalTrajectory:
    ...
    def test_KC7_realistic_timeline_still_offered(self, sci_module):
        """After flagging an unsafe rate, Kobe still gives a usable timeline
        (a date/projection), not just a 'no'."""
        out = _route(sci_module, "how long to 80 kg")
        assert "Weight timeline" in out

    def test_KC2_split_remaining_burn_per_workout_day(self, sci_module):
        """Prorated weekly target across remaining workout days — a per-day
        number, never a refusal or a carried 'debt'."""
        out = _route(sci_module, "I have 3 workouts left")
        assert "Per workout day" in out, (
            f"KC-2: no-debt split must return a per-day target; got {out[:200]}"
        )

    def test_KC1_remaining_block_has_a_number(self, sci_module):
        """KC-1: 'how much left this week' returns a Remaining block with a
        figure, never 'no idea'."""
        out = _route(sci_module, "how much do I have left for the week")
        assert "Remaining" in out
        assert re.search(r"\d", out), "remaining block must carry a number"


class TestKobeRefHRVEdgeCases:
    """KC-9 / KC-11 / KC-12 — HRV beyond a single band lookup."""

    @pytest.mark.parametrize("val,band", [
        (28, "RED"), (40, "YELLOW"), (55, "GREEN"), (90, "ELITE"),
    ])
    def test_KC9_band_classification_in_band(self, sci_module, val, band):
        """Unambiguous in-band values classify correctly (band gates the
        day's intensity — the highest-stakes call)."""
        out = _route(sci_module, f"hrv {val}").upper()
        assert band in out, f"KC-9: hrv {val} should be {band}; got {out[:120]}"

    def test_KC10_low_hrv_recommends_down_regulation(self, sci_module):
        out = _route(sci_module, "my hrv feels low").lower()
        assert "breathing" in out, "KC-10: low HRV must surface a protocol"

    @pytest.mark.xfail(
        strict=False,
        reason="KC-11 (KOBE_GEMINI_CHAT_REFERENCE): a 3-day run of low HRV "
               "(32/34/35) with rising RHR should be read as a downward "
               "TREND (deload), not three independent single-day lookups. "
               "Trend detection across logged readings is not implemented; "
               "the legacy band lookup is per-value only.",
    )
    def test_KC11_hrv_downward_trend_detected(self, sci_module):
        for v in (32, 34, 35):
            _route(sci_module, f"hrv {v}")
        out = _route(sci_module, "is my hrv trending down").lower()
        assert "trend" in out or "deload" in out or "pattern" in out, (
            "KC-11: a 3-day low-HRV run should be flagged as a trend"
        )

    def test_KC12_low_hrv_plus_short_sleep_yields_safe_outcome(self, sci_module):
        """KC-12: with low HRV + 4h sleep, the SAFE OUTCOME holds — the reply
        steers to recovery/rest and does not prescribe a hammer session.

        NOTE: this pins the safe *outcome* (which the low-HRV path already
        produces), not a dedicated sleep-debt model. True sleep-debt-overrides-
        burn-target reasoning (sleep as a first-class override of the calorie
        goal) is still future work — tracked in KC-12 of the reference doc."""
        out = _route(sci_module,
                     "I worked out but only slept 4 hours and HRV is low").lower()
        assert "recover" in out or "rest" in out or "sleep" in out
        assert "hammer" not in out, "must not prescribe intensity on 4h sleep"


class TestKobeRefMultiTurnArc:
    """KC-23 / KC-24 — the thread is a conversation, not single utterances."""

    @pytest.mark.xfail(
        strict=False,
        reason="KC-23 (KOBE_GEMINI_CHAT_REFERENCE): after 'I was sick Mon–Wed' "
               "the follow-up 'am I still on pace?' should carry the missed "
               "days into a prorated verdict. The legacy router answers each "
               "message statelessly (only DB-persisted facts carry over), so "
               "free-text missed-day context from a prior turn isn't folded "
               "into the pace math. Conversational memory is the new-plane "
               "chat-memory feature, not wired into Kobe's pace path yet.",
    )
    def test_KC23_missed_days_carry_into_pace(self, sci_module):
        _route(sci_module, "I was sick Mon to Wed and could not work out")
        out = _route(sci_module, "am I still on pace").lower()
        # A pace answer that reflects the missed days would mention being
        # behind / needing to catch up, not a generic on-track reply.
        assert "behind" in out or "catch" in out or "miss" in out

    def test_KC17_pace_verdict_is_structural(self, sci_module):
        """KC-17: even single-turn, the pace check returns the structural
        Today/target contract (the anchor Bug-H honesty rides on)."""
        out = _route(sci_module, "am I on track")
        assert "Today" in out
        assert "target" in out.lower() or "no target" in out.lower()
