...
def overlay(tmp_path: Path) -> Path:
    ...
    p.write_text(json.dumps({
        ...
        "one_rep_maxes_kg": {
            "deadlift": 155.0, "back_squat": 102.0,
...
def test_load_with_empty_db_and_no_overlay_returns_defaults(
    ...
    """With nothing on disk, loader returns defaults from the module's
    built-in overlay (Gemini-transcript-seeded)."""
    ...
    p = load()
    assert p.name == "Venkat"
    assert p.current_weight_lbs is None
    assert p.one_rep_maxes_kg, "should fall back to built-in 1RMs"
    assert "deadlift" in p.one_rep_maxes_kg
...
def test_load_applies_overlay(monkeypatch, empty_db, overlay):
    ...
    from core.user_profile import load
    p = load()
    assert p.one_rep_maxes_kg["deadlift"] == 155.0
...
def test_facts_block_has_required_sections(
    ...
    assert "deadlift: 155.0 kg / 342 lbs" in block
...
def test_get_1rm_lbs_returns_imperial(monkeypatch, empty_db, overlay):
    ...
    # 155 kg → 341.7 lbs
    assert abs(get_1rm_lbs("deadlift") - 341.7) < 0.5
...
def test_one_rep_maxes_lbs_converts(monkeypatch, empty_db, overlay):
    ...
    assert abs(lbs["deadlift"] - 341.7) < 0.5
    assert abs(lbs["back_squat"] - 224.9) < 0.5
