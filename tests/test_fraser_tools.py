...
3. `lookup_movement_cues` returns the canonical cues per category
   (pressing → neck-guard + HBP; squat → ankle-check + HBP; pull →
   hunch). Unknown movements get the HBP rule as a safe default.
...
def test_target_weight_snaps_down_to_plate_grid():
    """155 kg × 70% = 108.5 kg → snap DOWN to 107.5 (2.5-kg increment).
    Rounding-down is the safety contract: never program more than the
    math implies."""
    assert compute_target_weight("deadlift", 70, 155.0) == 107.5
...
def test_target_weight_lift_normalization():
    ...
    assert (compute_target_weight("DL", 70, 155.0)
            == compute_target_weight("Deadlifts", 70, 155.0)
...
def test_target_weight_zero_safe():
    ...
    assert compute_target_weight("deadlift", 0, 155.0) == 0.0
    assert compute_target_weight("deadlift", -5, 155.0) == 0.0
...
def test_olympic_lifts_get_all_three_categories():
    ...
    # Hunch (pull pattern), ankle-check (catch), HBP (heavy lift).
    assert HUNCH_CUE in cues
    assert ANKLE_CHECK_CUE in cues
    assert HBP_CUE in cues
...
def test_unknown_movement_gets_hbp_fallback():
    """Unknown movements get HBP as a safe default — the rule that
...
def test_parse_murph_at_70_percent_scales_reps():
    card = parse_user_workout(
        "Murph at 70%",
        one_rms_kg={"deadlift": 155.0, "back_squat": 130.0})
