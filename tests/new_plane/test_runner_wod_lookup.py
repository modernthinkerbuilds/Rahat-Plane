...
def test_lookup_defaults_to_today_when_no_day_specified(fake_adapter):
    ...
    # 2026-06-14: bare "what's my plan today" now delegates to kobe_route
    # (plan-query), bypassing the orchestrate path this test exercises.
    # @miya forces orchestrate (same convention as the sibling tests above)
    # so the no-day → no-gym_wod_on contract stays pinned on that path.
    resp = handle(Turn(user_message="@miya what's my plan today", chat_id="c1"))
