...
# 2026-06-14: "what's my plan today" now delegates to kobe_route, which
# the forced-HTTP fake adapter (adapter_client) cannot serve — it has no
# /route endpoint or kobe_route method (the B-P0 divergence; owner decision
# D2). The compare harness exists to diff old-vs-new SYNTHESIS on the
# orchestrate path, so probe with an open-coaching message that orchestrates
# on both planes. The HTTP-adapter route gap itself is pinned permanently by
# the native/adapter parity contract test (§1.2).
_COMPARE_PROBE = "how am I tracking toward my goal"


def test_compare_one_returns_both_sides(fake_adapter):
    r = compare_one(_COMPARE_PROBE)
    assert r.prompt == _COMPARE_PROBE
