...
LAYERS: list[LayerSpec] = [
    ...
    LayerSpec(
        ...
        description="Prompt injection / jailbreak / PII / hallucination probes "
                    "+ mined-corpus routing + voice-leak corpus.",
        paths=[
            "tests/evals/test_adversarial.py",
            # 2026-06-16: the tests/adversarial/ dir (mined-phrasing routing
            # corpus + voice-leak corpus) was never wired into the gate — it
            # only ran via direct pytest. Now it runs every check-in.
            "tests/adversarial/",
        ],
    ),
    LayerSpec(
    ...
    LayerSpec(
        name="quality_gate",
        description=("Every-check-in quality gate (2026-06-16): fast, "
                     "hermetic pins for the defect classes that shipped to "
                     "the user — hallucination, voice leak, wrong facts, "
                     "mis-routing, single-topic, no-invention. Run: "
