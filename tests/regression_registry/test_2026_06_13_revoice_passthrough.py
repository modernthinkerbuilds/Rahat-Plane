...
class TestKoboRouteIsRevoicedThroughSynth:
    ...
    def test_kobe_route_calls_synthesize_when_revoice_on(self, monkeypatch):
        ...
        # 2026-06-14: deterministic dispatcher routes now ship verbatim
        # (no re-voice). This test pins the OPEN-ENDED kobe path, so force
        # match_route → None to represent a non-deterministic reply.
        monkeypatch.setattr("core.dispatcher.match_route", lambda m: None)
        monkeypatch.setattr(
    ...
    def test_kobe_route_falls_back_when_synth_raises(self, monkeypatch):
        ...
        # Open-ended path (see note above) so the synth is actually invoked.
        monkeypatch.setattr("core.dispatcher.match_route", lambda m: None)
        monkeypatch.setattr(
            "new_plane.miya_runner.orchestrator.adapter.kobe_route",
            _fake_kobe_route,
