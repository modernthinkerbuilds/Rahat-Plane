...
def test_flag_on_daily_plan_sums_to_goal_target(env, monkeypatch):
    mod, _db = env
    ...
    assert total == round(st.weekly_target()), (
        f"daily ideals ({total}) must sum to the goal-derived weekly "
        f"target ({round(st.weekly_target())})")
