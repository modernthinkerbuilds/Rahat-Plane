...
def stub_handlers(monkeypatch):
    ...
    Hermeticity (2026-06-14): force RAHAT_COOLDOWN_LLM off. The repo `.env`
    ships `RAHAT_COOLDOWN_LLM=1` (a prod setting so cool-down / pre-fuel
    answers go through the personalized LLM), and `core.io` /
    `agents.the_scientist.handler` call `load_dotenv()` at import, leaking
    that flag into the test process. With it on, `dispatch()` *yields* the
    `pre_fuel` / `post_recovery` routes (returns None) so the LLM can
    answer — which makes the regex→route mapping assertions below
    nondeterministic. These tests pin which phrasings MATCH each route
    (flag-independent by design); the yield-on-flag behaviour is pinned
    separately in
    tests/regression_registry/test_2026_05_25_cooldown_yield_flag.py.
    """
