...
# ─────────────────────────── Subject seam (ADR-016, Seam 2) ──────────────
# ADDITIVE-ONLY. PM thesis §4 rule #1: "No hard-coded 'family' anywhere in
# the core. Every entity that holds preferences/state/data is a `Subject`
# with a unique ID and a role." Today this module is implicitly
# single-subject — `UserProfile.name` defaults to "Alex" (line ~80) and
# every loader reads the one vault DB with no subject scoping.
#
# `Subject` below is the typing seam for the SECOND subject (spouse,
# toddler, newborn → enterprise: customer/employee/account). It is defined
# but NOT yet consulted by any query. `DEFAULT_SUBJECT_ID` is the sentinel
# that means "today's behavior" — when `load(subject_id=...)` is called with
# the default, the loader behaves EXACTLY as before (no scoping, name
# "Alex"). See specs/ADR-016_platform_seams.md §"Seam 2 wiring" for where
# the per-subject DB filter plugs in (deliberately NOT wired here).
DEFAULT_SUBJECT_ID = "primary"


@dataclass(frozen=True)
class Subject:
    """A scoping key for profiles / state / signals (ADR-016 Seam 2).

    ADDITIVE-ONLY: nothing reads this yet. It exists so a 2nd subject can
    be added without forking the loader. `subject_id` is the stable unique
    key; `role` distinguishes the personal instantiation (self / spouse /
    child) from the enterprise one (customer / employee / account); `name`
    is the display name (today hard-coded as UserProfile.name="Alex").
    """
...
@dataclass
class UserProfile:
    """All the facts an agent needs to coach Alex without hallucinating.
    ...
    # ─── Identity ──────────────────────────────────────────────────────
    name: str = "Alex"
    # ADR-016 Seam 2 (additive): which Subject this profile is scoped to.
    # Defaults to the single-subject sentinel so existing callers and the
    # serialized shape are unchanged (the field is just carried, not yet
    # used to filter any query).
...
def load(subject_id: str = DEFAULT_SUBJECT_ID) -> UserProfile:
    ...
    ADR-016 Seam 2 (additive, backward-compatible):
        ``subject_id`` is OPTIONAL and defaults to ``DEFAULT_SUBJECT_ID``
        ("primary") — the single-subject sentinel. With the default, this
        function behaves EXACTLY as before: it reads the one vault DB with
        no scoping and stamps the profile's ``subject_id`` field with the
        sentinel. No query is changed.

        Per-subject SCOPING (passing a non-default subject_id to actually
        filter the DB rows) is intentionally NOT implemented here, because
        that *would* change behavior. Until the scoping is wired (see
        ADR-016 §"Seam 2 wiring"), a non-default subject_id is accepted and
        recorded on the profile but does not yet filter the data; callers
        must treat that as a known limitation, not a working feature. The
        signature is the seam; the filter is the future work.
    """
    db = _vault_db_path()
    p = UserProfile()
    p.subject_id = subject_id
    p.sources["db_path"] = str(db)
    p.sources["subject_id"] = subject_id
...
# A weigh-in older than this is flagged STALE in the prompt so the model
# caveats rather than quoting a number that may have drifted (round-2 P1-3).
_WEIGHT_STALE_WEEKS = 5


def _weight_staleness_note(weight_at: str | None,
                           *, now: "datetime | None" = None) -> str:
    """Return ' — STALE (…)' if the last weigh-in is older than
    _WEIGHT_STALE_WEEKS, else ''. Never raises — an unparseable timestamp
    yields no note (we don't fabricate staleness either)."""
    if not weight_at:
        return ""
    from datetime import datetime, timezone
    raw = weight_at.strip().replace("Z", "+00:00")
    parsed = None
    for fmt in (None, "%Y-%m-%d %H:%M:%S", "%Y-%m-%d"):
        try:
            parsed = (datetime.fromisoformat(raw) if fmt is None
                      else datetime.strptime(raw[:len(fmt) + 2], fmt))
            break
        except Exception:
            continue
    if parsed is None:
    ...
    weeks = (ref - parsed).days / 7.0
    if weeks > _WEIGHT_STALE_WEEKS:
        return (f" — STALE ({int(weeks)} weeks old; re-weigh before quoting "
                f"a current number)")
    return ""
...
def to_facts_block(p: UserProfile, *, include_diet: bool = True) -> str:
    ...
    if p.current_weight_lbs is not None:
        stale = _weight_staleness_note(p.current_weight_at)
        lines.append(
            f"  Current weight: {p.current_weight_lbs:.1f} lbs "
            f"(last logged {p.current_weight_at or 'unknown'}){stale}"
        )
    else:
