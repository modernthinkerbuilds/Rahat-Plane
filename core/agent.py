...
class Agent:
    ...
    # ─── round-2 P0-2: structured-output contract ───
    # The outbound validator (new_plane/miya_runner/validator.py) is the
    # SOLE content gate (the charter is content-blind). It is a strong but
    # phrasing-bounded backstop. Every agent registered AFTER the round-2
    # decision (PF-2026-06-17-002) must emit numeric facts as TYPED FIELDS,
    # not free text, so a fabricated number is structurally impossible and
    # the validator demotes to a backstop. Set this True and return facts
    # via a typed reply. The three grandfathered agents (kobe / fraser /
    # huberman) predate the contract and rely on the validator; the
    # tripwire test (tests/regression_registry/
    # test_2026_06_17_structured_output_contract.py) goes RED the moment a
    # NON-grandfathered agent registers with this still False.
    emits_structured_facts: bool = False
