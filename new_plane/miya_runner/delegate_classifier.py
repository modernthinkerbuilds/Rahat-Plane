...
# ─── Plan VIEW queries → Kobe (deterministic handle_show_plan) ─────────
# Bug 2026-06-14: "what is the plan for next week" / "which days am I
# working out next week" fell to orchestrate → the synth paraphrased a
# generic schedule, ignoring blacklists + the kcal target and giving a
# DIFFERENT answer on every ask. core.dispatcher already routes these to
# show_plan_this_week / show_plan_next_week deterministically — they just
# never reached it because the classifier sent them to orchestrate first.
# Routing them to kobe_route lets the dispatcher + handle_show_plan run.
_PLAN_QUERY_RE = re.compile(
    r"\b(?:"
    r"(?:what(?:'s|\s+is)?|show|see|tell\s+me)\s+(?:me\s+)?(?:my\s+|the\s+)?(?:plan|schedule)"
    r"|(?:this|next)\s+week(?:'s)?\s+(?:plan|schedule)"
    r"|(?:plan|schedule)\s+for\s+(?:this|next|the)\s+week"
    r"|which\s+days?\s+(?:am|are|do|will)\s+i\b"
    r"|what\s+(?:am\s+i|are\s+we)\s+doing\s+(?:this|next)\s+week"
    r")\b",
    re.I,
)
...
_STATUS_QUERY_RE = re.compile(
    ...
    r"(cal(?:orie)?s?|burn|kcal)(?:\s+\w+){0,4}\s+(by\s+(?:the\s+)?day|each\s+day|day[\s-]by[\s-]day)|"
    r"(by\s+(?:the\s+)?day|each\s+day|day[\s-]by[\s-]day)(?:\s+\w+){0,4}\s+(cal(?:orie)?s?|burn|kcal)|"
    r"daily\s+(?:burn\s+)?breakdown|"
    r"last\s+week|"
...
_PAIN_PROFILE_RE = re.compile(
    ...
    r"set\s+(my\s+)?(back\s*squat|front\s*squat|deadlift|squat|bench|press|snatch|clean)\s+(at|to|=)\s+\d+|"
    r"my\s+(1rm|max)\s+(for\s+)?\w+\s+is\s+\d+|"
    # NL 1RM set: "my back squat max is 120 kg", "deadlift is now 160".
    # Bug 2026-06-14 — these never reached Kobe's /profile set path.
    r"(my\s+)?(back\s*squat|front\s*squat|squat\s*clean|power\s*clean|bench\s*press|strict\s*press|push\s*press|deadlift|squat|bench|press|clean|snatch|ohp)\s+(1rm|max|pr)\s+(is|to|at|=)\s+(now\s+)?\d+|"
    r"(my\s+)?(back\s*squat|front\s*squat|deadlift|squat|bench|press|clean|snatch)\s+is\s+now\s+\d+"
    r")\b",
    re.I,
)
...
#   • workout_today — "today" collides with Fraser's daily-driver design and
#     with non-questions ("I did/want crossfit today").
#   • current_weight — bare "weigh" matches the weigh-in EVENT/timing
#     ("when should I weigh in") as well as the quantity. The exact weight
#     queries ("current weight", "what's my weight") are already owned by
#     the deterministic status patterns above, so nothing is lost.
# Both intents still resolve INSIDE Kobe's own route() once delegation has
# already chosen Kobe.
_DELEGATE_DIVERT_EXCLUDE = frozenset({"workout_today", "current_weight"})
...
def classify_delegation(msg: str) -> tuple[str, str]:
    ...
    # 5b. Plan VIEW queries → Kobe (handle_show_plan: blacklist + kcal,
    #     deterministic + consistent). Bug 2026-06-14.
    ...
    #     "what does my week look like", "what movements am I avoiding",
    #     "how much do I weigh these days". Without this, such a message
    #     falls to the orchestrate/synth path and gets PARAPHRASED into
    #     prose that disagrees with the deterministic `/plan` render (the
    #     2026-06-18 "two different answers" bug). Route it to Kobe so it
    #     lands on the SAME deterministic handler `/plan` uses, through the
    #     same finalize sink.
    #
    #     Safe by construction: flag-gated (RAHAT_INTENT_LAYER, default
    #     OFF → this whole block is skipped → byte-identical routing); the
    #     layer is read-only and abstains on doubt; and it sits LAST, so it
    #     can only divert a message that was already headed to
    #     "orchestrate" — never steal one an explicit pattern above owns.
    #     Honors the same design guard as step 8: a "design/build me a
    #     workout" request must still orchestrate so Fraser can author it,
    #     even though it contains read-intent words like "workout today".
    try:
        from core import intent_layer as _il
        if _il.enabled() and not _WOD_DESIGN_GUARD_RE.search(text):
            _il.ensure_registered()
            _m = _il.classify(text)
            # `workout_today` is excluded from DELEGATE-level diverting: at
            # this layer "today" phrasings are ambiguous with Fraser's
            # daily-driver design AND with non-questions ("I did crossfit
            # today", "I want to do crossfit today"), which should stay on
            # the orchestrate/synth path. The intent still works INSIDE
            # Kobe's route() once delegation has already chosen Kobe.
            if _m is not None and _m.name not in _DELEGATE_DIVERT_EXCLUDE:
                return ("kobe_route", text)
    except Exception:
        # Never let the intent layer break delegation — fall through to
