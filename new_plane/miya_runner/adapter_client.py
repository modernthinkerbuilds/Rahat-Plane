...
# ─── Full-route delegation (parity with native_client) ───────────────────
# Mirror native_client.{kobe,fraser,huberman}_route over HTTP so the
# orchestrator's delegation paths also work in HTTP mode
# (NEW_MIYA_USE_HTTP_CLIENT=1). Before this, adapter_client lacked these and
# `adapter.kobe_route(...)` raised AttributeError in HTTP mode. Server
# endpoints: /kobe/route, /fraser/route, /huberman/route.
def kobe_route(message: str, chat_id: str | None = None,
               trace_id: str | None = None) -> AdapterResult:
    return post("/kobe/route", {"message": message, "chat_id": chat_id},
                trace_id=trace_id)


def fraser_route(message: str, chat_id: str | None = None,
                 trace_id: str | None = None) -> AdapterResult:
    return post("/fraser/route", {"message": message, "chat_id": chat_id},
                trace_id=trace_id)


def huberman_route(message: str, chat_id: str | None = None,
                   trace_id: str | None = None) -> AdapterResult:
    return post("/huberman/route", {"message": message, "chat_id": chat_id},
                trace_id=trace_id)
